      ...
      <div class="add-info-box">
        ...
        <div class="car-btn-group">
          ...
          <div class="car-btn-row">
            <router-link :to="{path: '/goodsDetail', query: { goodsId: newShoppingCart.relProductId, merchantId: 1, relCategory3Id: newShoppingCart.categoryId}}">
            <button class="btn-car btn-car-to-detail">查看商品详情</button>
            </router-link>
            <button class="btn-car btn-car-to-pay" @click="goToOrder()">去购物车结算 > </button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="other-user-buy-row" v-for="(items,index1) in recommend" :key="index1">
        <div class="other-user-buy-item-box" v-for="(item,index2) in items" :key="index2">
          <div class="other-user-buy-item-img">
            <router-link :to="{path: '/goodsDetail', query: { goodsId: item.id, merchantId: 1, relCategory3Id: item.relCategory3Id}}">
            ...
            <div class="other-buy-btn-box">
              <router-link :to="{path: '/goodsDetail', query: { goodsId: item.id, merchantId: 1, relCategory3Id: item.relCategory3Id}}">
                <button class="other-buy-btn" >
                  <Icon type="ios-cart"></Icon> 商品详情</button>
              </router-link>
...
export default {
  ...
  created () {
    if (this.newShoppingCart === null || this.newShoppingCart === undefined || this.newShoppingCart.length === 0){
      return
    }
